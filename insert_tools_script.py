#!/usr/bin/env python3
"""
insert_tools_script.py

Insert agents into Chroma and tools into Mongo + Chroma.

Usage:
  - Import this module and call insert_all_agents(agents_list)
  - or run directly to insert ALL_AGENTS and ALL_TOOLS if available in the project

Behavior:
  - Agents: inserted into Chroma agents collection using chroma_inserter.insert_agent()
  - Tools: metadata inserted/updated in MongoDB tools collection via mongo_inserter.insert_tool()
           embeddings + light metadata inserted into Chroma tools collection via chroma_inserter.insert_tool()
"""
...
# factories for inserters (must be in project)
...
def insert_agent(agent: Dict[str, Any]) -> bool:
    """
    Insert a single agent into Chroma (agents collection).
...
def insert_all_agents(all_agents: List[Dict[str, Any]]) -> bool:
    """
    Batch insert all agents; returns True if all successful, False otherwise.
    """
    logger.info(f"Starting batch insert of {len(all_agents)} agents")
    for i, agent in enumerate(all_agents, start=1):
        logger.info(f"[{i}/{len(all_agents)}] Inserting agent: {agent.get('agent_id')}")
        if not insert_agent(agent):
            logger.error(f"Stopping batch on failure inserting agent: {agent.get('agent_id')}")
            return False
        # small delay helps avoid throttling in remote services
        time.sleep(0.05)
    logger.info("All agents inserted successfully")
    return True


def insert_all_tools(all_tools: List[Dict[str, Any]]) -> bool:
    """
    Batch insert all tools; returns True if all successful, False otherwise.
    """
    logger.info(f"Starting batch insert of {len(all_tools)} tools")
    for i, tool in enumerate(all_tools, start=1):
        logger.info(f"[{i}/{len(all_tools)}] Inserting tool: {tool.get('tool_id')}")
        if not insert_tool(tool):
            logger.error(f"Stopping batch on failure inserting tool: {tool.get('tool_id')}")
            return False
        time.sleep(0.05)
    logger.info("All tools inserted successfully")
    return True
...
if __name__ == "__main__":
    ...
    # Try to import project constants. If unavailable, exit gracefully.
    try:
        from agent_structure import ALL_AGENTS
    except Exception:
        ALL_AGENTS = []

    # Many codebases used neo4j_tool_structure.py variable name ALL_TOOLS_ENHANCED
    try:
        from neo4j_tool_structure import ALL_TOOLS_ENHANCED as ALL_TOOLS
    except Exception:
        # fallback to module named tool_definitions or similar
        try:
            from api_template import ALL_TOOLS_ENHANCED as ALL_TOOLS
        except Exception:
            ALL_TOOLS = []

    # Perform insertion based on available data
    if not ALL_AGENTS and not ALL_TOOLS:
        print("No agents or tools found to insert. Add ALL_AGENTS and ALL_TOOLS_ENHANCED to the project or call the functions programmatically.")
        sys.exit(0)

    if ALL_AGENTS:
        ok = insert_all_agents(ALL_AGENTS)
        if not ok:
            logger.error("Agent insertion batch failed")
            sys.exit(1)

    if ALL_TOOLS:
        ok2 = insert_all_tools(ALL_TOOLS)
        if not ok2:
            logger.error("Tools insertion batch failed")
            sys.exit(1)

    logger.info("Insert script completed successfully")
    sys.exit(0)

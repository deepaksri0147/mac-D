#!/usr/bin/env python3
"""
multi_step_tool_workflow.py

End-to-end workflow:
  1. Discover agents (Chroma agents collection) using AgentWorkflow
  2. Discover tools (Chroma tools collection -> Mongo metadata) using ToolWorkflow
  3. Return a combined result and optionally run a demo execution

Dependencies:
  - agent_workflow.AgentWorkflow
  - tool_workflow.ToolWorkflow
  - chroma_inserter.create_chroma_inserter (for some admin ops if needed)
  - mongo_inserter.create_mongo_tool_inserter
  - embedding_service_ollama.create_ollama_embedding_service
"""
...
class MultiStepToolWorkflow:
    """
    Orchestrates agent discovery -> tool discovery -> optional execution.
    """
    ...
    def run_workflow(
        ...
        agent_top_k: int = 5,
        tool_top_k: int = 8,
        rerank_agents_with_llm: bool = True,
        rerank_tools_with_llm: bool = True,
        execute_with_agent: bool = False
    ) -> Dict[str, Any]:
        """
        Run end-to-end workflow.

        Returns a dict with keys:
          - agent_candidates: list of agent candidate dicts (agent_id + metadata if available)
          - ranked_agents: list of ranked agent_ids (ordered)
          - selected_agent: top agent id (or None)
          - tools_result: result returned by ToolWorkflow.run (selected_tools + fallback flag)
          - execution_result: (optional) result of demo agent execution if execute_with_agent=True
        """
        logger.info("Starting MultiStepToolWorkflow")
        output: Dict[str, Any] = {
            "agent_candidates": [],
            "ranked_agents": [],
            "selected_agent": None,
        ...
        # 1) Agent discovery (Chroma agents collection)
        try:
            ranked_agents = self.agent_wf.run_workflow(user_query, top_k=agent_top_k, rerank_with_llm=rerank_agents_with_llm)
            logger.info(f"Agent discovery returned: {ranked_agents}")
        except Exception as e:
            logger.error(f"Agent discovery failed: {e}", exc_info=True)
            ranked_agents = []

        output["ranked_agents"] = ranked_agents
        output["selected_agent"] = ranked_agents[0] if ranked_agents else None

        if not ranked_agents:
            logger.warning("No agents found; skipping tool discovery")
            return output

        # 2) Tool discovery (Chroma tools collection -> Mongo metadata)
        try:
            tools_result = self.tool_wf.run(user_query, top_k=tool_top_k, rerank_with_llm=rerank_tools_with_llm)
            logger.info("Tool discovery completed")
            output["tools_result"] = tools_result
        except Exception as e:
            logger.error(f"Tool discovery failed: {e}", exc_info=True)
            output["tools_result"] = {"selected_tools": [], "fallback_used": False}

        # 3) (Optional) Execute selected tool(s) via agent (demo)
        if execute_with_agent and output["tools_result"]:
            try:
                tools = output["tools_result"].get("selected_tools", [])
                if tools:
                    exec_res = self.tool_wf.execute_with_agent(user_query, tools)
                    output["execution_result"] = exec_res
                else:
                    logger.info("No selected tools to execute")
                    output["execution_result"] = None
            except Exception as e:
                logger.error(f"Execution step failed: {e}", exc_info=True)
                output["execution_result"] = {"error": str(e)}
...
if __name__ == "__main__":
    ...
    ms = MultiStepToolWorkflow()

    query = "create schema named Users with attributes email as string and age as number"
    result = ms.run_workflow(
        query,
        agent_top_k=5,
        tool_top_k=6,
        rerank_agents_with_llm=True,
        rerank_tools_with_llm=True,
        execute_with_agent=False
    )
